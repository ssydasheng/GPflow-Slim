...
class GridInteprolation(object):
    ...
    def inducing_forward(self):
        covs = []
        for id in range(len(self.grid_bounds)):
            cov = self.kernel.Kdim(id, np.expand_dims(self.grid[id], 1))
            covs.append(cov)
        return Kronecker(covs)

    def compute_interpolation(self, inputs):
        """
        :param inputs: [n, d]
        :return: sparse interpolation matrix of shape [n, grid_size ** d]
        """
        return Interpolation().interpolate(self.grid, inputs)
...
class Interpolation(object):
    def _cubic_interpolation_kernel(self, scaled_grid_dist):
        ...
        """
        U = np.abs(scaled_grid_dist)
        res = np.zeros_like(U, dtype=settings.tf_float)

        U_lt_1 = np.less(U, 1).astype(settings.float_type)
    ...
    def interpolate(self, x_grid, x_target, interp_points=range(-2, 2)):
        ...
        num_grid_points = np.shape(x_grid)[1]
        num_target_points = np.shape(x_target)[0]
        num_dim = np.shape(x_grid)[0]
        num_coefficients = len(interp_points)


        interp_points_flip = np.array(interp_points[::-1], dtype=settings.float_type)
        interp_points =  np.array(interp_points, dtype=settings.float_type)

        interp_values = np.ones([num_target_points, num_coefficients ** num_dim], dtype=settings.tf_float)
        interp_indices = np.zeros([num_target_points, num_coefficients ** num_dim], dtype=settings.int_type)

        for i in range(num_dim):
            grid_delta = x_grid[i, 1] - x_grid[i, 0]
            lower_grid_pt_idxs = np.squeeze(np.floor((x_target[:, i] - x_grid[i, 0]) / grid_delta))
            lower_pt_rel_dists = (x_target[:, i] - x_grid[i, 0]) / grid_delta - lower_grid_pt_idxs
            lower_grid_pt_idxs = lower_grid_pt_idxs - np.max(interp_points)

            scaled_dist = np.expand_dims(lower_pt_rel_dists, -1) + np.expand_dims(interp_points_flip, -2)
            ...
            left_boundary_pts = np.where(lower_grid_pt_idxs < 1)[0]
            num_left = np.shape(left_boundary_pts)[0]

            ## only support eager mode for now.
            if num_left > 0:
                x_grid_first = np.tile(np.transpose(np.expand_dims(x_grid[i, :num_coefficients], 1)), [num_left, 1])
                grid_targets = np.tile(np.expand_dims(x_target[left_boundary_pts, i], 1), [1, num_coefficients])
                dists = np.abs(x_grid_first - grid_targets)
                closest_from_first = np.argmin(dists, 1)
            ...
            right_boundary_pts = np.where(lower_grid_pt_idxs > num_grid_points - num_coefficients)[0]
            num_right = len(right_boundary_pts)

            if num_right > 0:
                x_grid_last = np.tile(np.transpose(np.expand_dims(x_grid[i, -num_coefficients:], 1)), [num_right, 1])
                grid_targets = np.tile(np.expand_dims(x_target[right_boundary_pts, i], 1), [1, num_coefficients])
                dists = np.abs(x_grid_last - grid_targets)
                closest_from_last = np.argmin(dists, 1)
            ...
            offset = np.expand_dims(interp_points - np.min(interp_points), -2)
            dim_interp_indices = np.expand_dims(lower_grid_pt_idxs, -1) + offset

            n_inner_repeat = num_coefficients ** i
            n_outer_repeat = num_coefficients ** (num_dim - i - 1)
            index_coeff = num_grid_points ** (num_dim - i - 1)

            dim_interp_indices = np.tile(np.expand_dims(dim_interp_indices, -1), [1, n_inner_repeat, n_outer_repeat])
            dim_interp_values = np.tile(np.expand_dims(dim_interp_values, -1), [1, n_inner_repeat, n_outer_repeat])
            interp_indices = interp_indices + np.reshape(dim_interp_indices, [num_target_points, -1]) * index_coeff
            interp_values = interp_values * np.reshape(dim_interp_values, [num_target_points, -1])
